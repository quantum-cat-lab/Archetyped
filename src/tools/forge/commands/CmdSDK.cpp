...
// sync-sdk
// =============================================================================
int CmdSyncSdk::execute(const Args& args, Workspace& ws) {
    ...
    json wsJson = ws.loadWorkspaceJson();
    if (!wsJson.contains("modules") || !wsJson["modules"].is_array()) {
        std::cerr << "forge: error: module_workspace.json has no 'modules'\n";
        return 1;
    }
    ...
    for (auto& [name, vers] : byName) {
        ...
        if (distinct.size() > 1) {
            std::cerr << "forge: conflict: version conflict for SDK unit '" << name << "'\n";
            conflict = true;
        }
        for (auto& [ver, mods] : verMods)
            if (mods.size() > 1) {
                std::cerr << "forge: conflict: ambiguous provider for '" << name << "@" << ver << "'\n";
                conflict = true;
            }
    }
        ...
        if (sm.contains("requires") && sm["requires"].is_array()) {
            for (auto& req : sm["requires"]) {
                ...
                if (!findProvider(byName, rname, rver, pMod, pPath, headers)) {
                    std::cerr << "forge: conflict: module '" << mname << "' requires '"
    ...
    if (problem && !allMode) return 1;

    if (listMode) {
        std::cout << "forge sync-sdk --list (workspace: " << ws.modulesRoot() << ")\n";
    ...
    if (checkMode) {
        std::cout << "forge sync-sdk --check (workspace: " << ws.modulesRoot() << ")\n";
        bool ok = true;
        for (auto& [mname, specs] : resolved) {
            for (auto& s : specs) {
        ...
        if (!ok) { std::cerr << "forge: --check FAILED (drift detected)\n"; std::exit(1); }
        cli::printSuccess("--check PASSED (all vendored copies match canonical)");
        return 1;
    }

    int written = 0, skipped = 0;
    std::cout << "forge sync-sdk (workspace: " << ws.modulesRoot() << ")\n";
    for (auto& [mname, specs] : resolved) {
        for (auto& s : specs) {
            ...
            for (auto& hrel : s.headers) {
                ...
                fs::path dst = vendorDest(s.modulePath, s.unit, hrel);
                if (!fs::exists(src)) {
                    std::cerr << "forge: error: canonical header missing: " << src << "\n";
                    return 1;
                }
            ...
            mgContent += "// Auto-generated by forge sync-sdk. DO NOT EDIT MANUALLY.\n";
...
// check-sdk
// =============================================================================
int CmdCheckSdk::execute(const Args& args, Workspace& ws) {
    (void)args;
    std::cout << "forge check-sdk (workspace: " << ws.modulesRoot() << ")\n\n";
    bool allOk = true;
...
// init-sdk
// =============================================================================
int CmdInitSdk::execute(const Args& args, Workspace& ws) {
    ...
    try {
        ...
        std::cout << "    3. Run 'forge sync-sdk' to vendor headers into consumers\n";
...
// deps
// =============================================================================
int CmdDeps::execute(const Args& args, Workspace& ws) {
