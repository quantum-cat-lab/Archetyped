...
int CmdModuleList::execute(const Args& args, Workspace& ws) {
    (void)args;
...
int CmdModuleInfo::execute(const Args& args, Workspace& ws) {
    std::string modName = args.pos(0);
    if (modName.empty()) { cli::printError("ModuleInstance name required"); return 1; }

    fs::path modPath = ws.resolveModule(modName);
    if (modPath.empty()) { cli::printError("ModuleInstance '" + modName + "' not found"); return 1; }

    ModuleInfo mi = ws.loadModule(modPath);
    if (mi.id.empty()) { cli::printError("Failed to read module.json for '" + modName + "'"); return 1; }
...
int CmdModuleInit::execute(const Args& args, Workspace& ws) {
    std::string name = args.pos(0);
    if (name.empty()) { std::cerr << "Error: ModuleInstance name required.\n"; return 1; }

    bool withCSharp = args.has("--csharp") || args.has("--with-csharp");
    bool csharpOnly = args.has("--csharp-only");
    ...
    try {
        ...
        if (fs::exists(tmpl)) {
            ...
            if (fs::exists(modPath / "Arche.ModuleInstance.csproj.template"))
                fs::remove(modPath / "Arche.ModuleInstance.csproj.template");
            if (fs::exists(modPath / "csharp-template"))
                fs::remove_all(modPath / "csharp-template");

            fs::create_directories(modPath / "assets");
            fs::create_directories(modPath / "configs");

            if (withCSharp || csharpOnly) {
                fs::path csTpl = tmpl / "Arche.ModuleInstance.csproj.template";
                if (fs::exists(csTpl)) {
                    std::ifstream in(csTpl);
                    std::string content((std::istreambuf_iterator<char>(in)), std::istreambuf_iterator<char>());
                    size_t pos = 0;
                    while ((pos = content.find("{{MODULE_NAME}}", pos)) != std::string::npos) {
                        content.replace(pos, std::string("{{MODULE_NAME}}").size(), name);
                        pos += name.size();
                    }
                    std::string csProjName = "Arche." + name + ".csproj";
                    std::ofstream out(modPath / csProjName);
                    out << content;
                }
                fs::path entryTpl = tmpl / "csharp-template" / "Entry.cs.template";
                if (fs::exists(entryTpl)) {
                    std::ifstream in(entryTpl);
                    std::string content((std::istreambuf_iterator<char>(in)), std::istreambuf_iterator<char>());
                    size_t pos = 0;
                    while ((pos = content.find("{{MODULE_NAME}}", pos)) != std::string::npos) {
                        content.replace(pos, std::string("{{MODULE_NAME}}").size(), name);
                    ...
                    fs::path outDir = modPath / "src" / "main" / "csharp" / ("Arche." + name);
                    fs::create_directories(outDir);
                    std::ofstream out(outDir / "Entry.cs");
                    out << content;
                }
            }

            if (csharpOnly) {
                if (fs::exists(modPath / "CMakeLists.txt")) fs::remove(modPath / "CMakeLists.txt");
                if (fs::exists(modPath / "src" / "module.cpp")) fs::remove(modPath / "src" / "module.cpp");
            }
            ...
            if (withCSharp || csharpOnly) {
                modJson["csharp"] = {
                    {"csproj", "Arche." + name + ".csproj"},
                    {"entry_type", "Arche." + name + ".Entry"}
                };
            }
            ...
            if (csharpOnly) cli::printSuccess("ModuleInstance '" + name + "' created (C# only) from template");
            else if (withCSharp) cli::printSuccess("ModuleInstance '" + name + "' created (hybrid C++ + C#) from template");
            else cli::printSuccess("ModuleInstance '" + name + "' created from template");
            return 1;
        }
...
int CmdModuleUpdate::execute(const Args& args, Workspace& ws) {
    std::string name = args.pos(0);
    if (name.empty()) { std::cerr << "Error: ModuleInstance name required.\n"; return 1; }
...
int CmdModuleScan::execute(const Args& args, Workspace& ws) {
    (void)args;
