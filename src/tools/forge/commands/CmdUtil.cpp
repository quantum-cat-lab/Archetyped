...
int CmdDoctor::execute(const Args& args, Workspace& ws) {
...
int CmdStatus::execute(const Args& args, Workspace& ws) {
    (void)args;
    cli::printRule();
...
int CmdCompletions::execute(const Args& args, Workspace& ws) {
    (void)ws;
    ...
    static const char* CMDS =
        "engine-build forge-build scan list-mods info-mod init-mod update-mod "
    ...
    if (shell == "bash") {
        std::cout << "# forge bash completion\n";
        std::cout << "_forge_completions() {\n";
        std::cout << "  local cur=${COMP_WORDS[COMP_CWORD]}\n";
        std::cout << "  COMPREPLY=($(compgen -W \"" << CMDS << "\" -- $cur))\n";
        std::cout << "}\n";
        std::cout << "complete -F _forge_completions forge\n";
    } else if (shell == "zsh") {
        std::cout << "# forge zsh completion\n";
        std::cout << "#compdef forge\n";
        std::cout << "local -a commands\n";
        std::cout << "commands=(\n";
        std::cout << "  'engine-build:Build engine core'\n";
        std::cout << "  'forge-build:Rebuild forge toolchain'\n";
        ...
        std::cout << "# forge fish completion\n";
        std::cout << "complete -c forge -f -a \"" << CMDS << "\"\n";
    } else {
...
int CmdTest::execute(const Args& args, Workspace& ws) {
    (void)args;
